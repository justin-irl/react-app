pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("replicamask/react-app")
                    app.inside {
         ...
         stage('DeployToStaging') {
            ...
            steps {
                    script {
                        ...
                        sh "docker run --restart always --name react-app -p 1233:80 -d replicamask/react-app:${env.BUILD_NUMBER}"
                    }
            }
        }
        ...
        stage('DeployToProduction') {
            ...
            steps {
                input 'Does the staging environment look OK? Did You get 200 response?'
                 milestone(1)
                    script {
                        sh "docker pull replicamask/react-app:${env.BUILD_NUMBER}"
                        try {
                            sh "docker stop react-app"
                            sh "docker rm react-app"
                        } catch (err) {
